...
export async function getAllLessonChild(pathString: string) {
	...
	return await Promise.all(
		page_routes_copy.map(async (it) => {
			const totalPath = path.join(process.cwd(), '/contents/lessons/', prevHref, it.href, 'index.mdx');
...
export async function getAllAnnouncementStaticPaths() {
	try {
		const announcementFolder = path.join(process.cwd(), '/contents/announcements/');
...
export async function getAllAnnouncements() {
	...
	const uncheckedRes = await Promise.all(
		files.map(async (file) => {
			...
			const filepath = path.join(process.cwd(), `/contents/announcements/${file}`);
...
export async function getAnnouncementForSlug(slug: string) {
	const announcementFile = path.join(process.cwd(), '/contents/announcements/', `${slug}.mdx`);
	try {
