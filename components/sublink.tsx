...
export default function SubLink({
  ...
  const titleOrLink = !noLink ? (
    ...
    <h4 className="font-medium text-primary sm:text-sm">{title}</h4>
  );
    ...
    <div className="flex w-full flex-col gap-1">
      <Collapsible open={isOpen} onOpenChange={setIsOpen}>
          ...
          <div className="flex w-full cursor-pointer items-center justify-between">
            ...
            <span>
              {!isOpen ? (
                ...
                <ChevronDown className="h-[0.9rem] w-[0.9rem] text-primary" />
              )}
            </span>
          </div>
        </CollapsibleTrigger>
        <CollapsibleContent>
          <div
            className={cn(
              "ml-0.5 mt-2.5 flex flex-col items-start gap-2 text-stone-800 dark:text-stone-300/85 sm:text-sm",
              level > 0 && "ml-1.5 border-l pl-4"
            )}
          >
            {items?.map((innerLink) => {
              const modifiedItems = {
